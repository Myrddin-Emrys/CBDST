...
# In[4]: Dropping the columns that do not refer to respondents soc.-dem. characteristics, creating clusterization dataset ('get_id_2')
...
# In[8]: Testing FIRST algorithm. The main idea is to get the ID by simply aggregating all the values (of soc.-dem. characteristics) by the row in a string variable.
# To do this all categorial values were converted to numbers (numerated in oreder of their appeaarance in the dataset).
...
# In[10]: The result is an ID for each row, consisting of string value which is composed from all the numerical values from categorical data, representing combination of soc.-dem characteristics
...
# In[12]: As OPTICS works with categorical data, all the values (recorded as numbers, but then converted to strings for previous step) were converted to numeric values.
...
# In[15]: Testing the THIRD algorithm. It's the sorting function, which goes through each column, sorting them by values and labeling each sorted group. 
# As a results we get the labelled groups of all possible sorting combinations by each column. Which is believed to be human-like way to identify unique combinations of soc.-dem. characteristics of each individaual.
...
# In[18]: In order to compare accuracy dataset of obtained labels was sorted by values of target (need to be predicted) variable. For each group the median values of labels returned by each algorithm were calculated.
...
# In[19]: Accuracy were estimated by the rate of correspondence of median label of each algorithm for each target variable group (rows referred to unique individual) and labels returned by algorithms
